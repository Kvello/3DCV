...
def detect_lines(image: np.ndarray) -> np.ndarray:
    ...
    """
    image = cv2.Canny(image, threshold1=MIN_CANNY_THRESHOLD,
                      threshold2=MAX_CANNY_THRESHOLD, apertureSize=3)
    line_pts = cv2.HoughLinesP(image, rho=1, theta=np.pi/180, threshold=100,
...
def get_pairwise_intersections(lines: np.ndarray) -> np.ndarray:
    ...
    num_intersections = int(num_lines*(num_lines-1)/2)
    intersections = np.empty(
        shape=[num_intersections, 3], dtype=lines.dtype)
    """ YOUR CODE HERE """
    indx = 0
    for i in range(num_lines):
        for j in range(i+1, num_lines):
            intersections[indx] = np.cross(lines[i], lines[j])
            indx += 1
    no_intersections = np.where(intersections[:, 2] == 0)
    intersections = np.delete(intersections, no_intersections, axis=0)
    intersections = intersections / intersections[:, 2].reshape(-1, 1)
    """ END YOUR CODE HERE """
...
def get_support_mtx(intersections: np.ndarray, lines: np.ndarray, distance_threshold: float = DISTANCE_THRESHOLD):
    ...
    for i in range(num_intersections):
        for j in range(num_lines):
            support_mtx[i, j] = (np.abs(np.dot(lines[j], intersections[i])) / np.sqrt(
                lines[j][0]**2 + lines[j][1]**2)) < distance_threshold
    """ END YOUR CODE HERE """
...
def get_vanishing_pts(lines: np.ndarray, num_vanishing_pts: int) -> np.ndarray:
    ...
    for _ in range(num_vanishing_pts):
        print("Support matrix: ", support_mtx)
        support_sum = np.sum(support_mtx, axis=1)
        vanishing_pt_idx = np.argmax(support_sum)
        print("Vanishing point index: ", vanishing_pt_idx)
        vanishing_pts.append(intersections[vanishing_pt_idx])
        support_set = np.where(support_mtx[vanishing_pt_idx] == 1)
        support_mtx[:, support_set] = 0
    """ END YOUR CODE HERE """
...
def _parse_info_dict(info: dict):
    ...
    top, btm = np.ones(shape=3, dtype=np.float32), np.ones(
...
def get_target_height(vanishing_line: np.ndarray, query_info: dict, target_info: dict, vanishing_pt_v: np.ndarray):
    ...
    """
    inter_line = np.cross(pt_b1, pt_b2)
    target_line = np.cross(pt_t2, pt_b2)
    vanishing_pt_u = np.cross(vanishing_line, inter_line)
    vanishing_pt_u = vanishing_pt_u / vanishing_pt_u[2]
    transferred_pt = np.cross(np.cross(vanishing_pt_u, pt_t1), target_line)
    transferred_pt = transferred_pt / transferred_pt[2]
    vanishing_pt_v = vanishing_pt_v / vanishing_pt_v[2]
    distance_transferred = np.linalg.norm(pt_b2-transferred_pt)
    distance_top = np.linalg.norm(pt_b2-pt_t2)
    distance_v = np.linalg.norm(pt_b2-vanishing_pt_v)
    print("Query point t1: ", pt_t1, "Query point b1: ", pt_b1)
    print("Target point t2: ", pt_t2, "Target point b2: ", pt_b2)
    print("Vanishing line: ", vanishing_line, "Inter line: ", inter_line,
          "Target line: ", target_line)
    print("Transferred point: ", transferred_pt, "Vanishing point u: ",
          vanishing_pt_u, "Vanishing point v: ", vanishing_pt_v)
    print("Distance transferred: ", distance_transferred,
          "Distance top: ", distance_top, "Distance v: ", distance_v)
    target_height = distance_top*(distance_v-distance_transferred) / \
        (distance_transferred*(distance_v-distance_top))*query_d1
...
def main():
    ...
    vert_vanishing_pt = vert_vanishing_pt / vert_vanishing_pt[2]
    horizontal_vanishing_pts[0] = horizontal_vanishing_pts[0] / \
        horizontal_vanishing_pts[0][2]
    horizontal_vanishing_pts[1] = horizontal_vanishing_pts[1] / \
        horizontal_vanishing_pts[1][2]
