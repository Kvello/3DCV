...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    """
    X_3d_hom = np.concatenate([points3d[points3d_idxs],np.ones((points3d[points3d_idxs].shape[0],1))],axis=1)
    x_2d_hom_reproj = np.matmul(extrinsics[camera_idxs],X_3d_hom.reshape((-1,4,1))).squeeze(2)
    x_2d_hom_reproj = np.matmul(x_2d_hom_reproj,intrinsics.T)
    x_2d_reproj = x_2d_hom_reproj[:,:2]/x_2d_hom_reproj[:,2].reshape(-1,1)
    residuals = (points2d-x_2d_reproj).reshape(-1)
    """ END YOUR CODE HERE """
    return residuals
