...
def decode_keypoint(kp: tuple) -> cv2.KeyPoint:
    ...
    # Note: cv2.KeyPoint(x, y, _size, _angle, _response, _octave, _class_id)
    # had to change the variable names
...
def create_feature_matches(image_file1: os.path, image_file2: os.path, lowe_ratio: float = 0.6, min_matches: int = 10):
    ...
    for n,m in matches:
        if n.distance < lowe_ratio * m.distance:
            good_matches.append([n])
    # Alternatively we can use crossCheck to filter out the matches
...
def create_scene_graph(image_files: list, min_num_inliers: int = 40):
    ...
    for i in range(len(image_files)):
        for j in range(i+1, len(image_files)):
            match_id = '{}_{}'.format(image_ids[i], image_ids[j])
            match_file = os.path.join(RANSAC_MATCH_DIR, match_id + '.npy')
            if os.path.exists(match_file):
                inliers_idx = np.load(match_file)
                num_inliners = inliers_idx.shape[0]
                if(num_inliners > max_inliners):
                    max_inliners = num_inliners
                if inliers_idx.shape[0] >= min_num_inliers:
                    graph.add_edge(i, j)
                    graph.add_edge(j, i)
    """ END YOUR CODE HERE """
...
def main():
    ...
    ## Result verification
    TA_DIR = os.path.join(PROJECT_DIR, 'ta-results')
    TA_RESULT_DIR = os.path.join(TA_DIR, DATASET)
    TA_RANSAC_MATCH_DIR = os.path.join(TA_RESULT_DIR, 'ransac-match')
    TA_RANSAC_ESSENTIAL_DIR = os.path.join(TA_RESULT_DIR, 'ransac-fundamental')
    ## Find the corresponding files in the the local results from the TA results
    essential_matrix_error = 0
    for file in os.listdir(TA_RANSAC_ESSENTIAL_DIR):
        if file.endswith('.npy'):
            ta_essential_matrix = np.load(os.path.join(TA_RANSAC_ESSENTIAL_DIR, file))
            essential_matrix = np.load(os.path.join(RANSAC_ESSENTIAL_DIR, file))
            essential_matrix_error += np.sum(np.abs(ta_essential_matrix - essential_matrix))
    inliner_num_error = 0
    for file in os.listdir(TA_RANSAC_MATCH_DIR):
        if file.endswith('.npy'):
            ta_inliers = np.load(os.path.join(TA_RANSAC_MATCH_DIR, file))
            inliers = np.load(os.path.join(RANSAC_MATCH_DIR, file))
            inliner_num_error += np.abs(ta_inliers.shape[0] - inliers.shape[0])
    print(f'Essential Matrix Error: {essential_matrix_error}')
    print(f'Inlier Number Error: {inliner_num_error}')
