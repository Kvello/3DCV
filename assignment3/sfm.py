""" CS4277/CS5477 Lab 3: STRUCTURE FROM MOTION AND BUNDLE ADJUSTMENT 
...
def get_init_image_ids(scene_graph: dict) -> (str, str):
    ...
    """ YOUR CODE HERE """
    max_matches = 0
    for im_id, neighbors in scene_graph.items():
        for neighbor in neighbors:
            if neighbor < im_id: #Only consider pairs once
                continue
            match_id = '_'.join(sorted([im_id, neighbor]))
            match_file = os.path.join(RANSAC_MATCH_DIR, match_id + '.npy')
            matches = np.load(match_file)
            if len(matches) > max_matches:
                max_matches = len(matches)
                max_pair = [im_id, neighbor]
    """ END YOUR CODE HERE """
...
def get_init_extrinsics(image_id1: str, image_id2: str, intrinsics: np.ndarray) -> (np.ndarray, np.ndarray):
    ...
    # There are 4 possible solutions for the extrinsics. Need to find the correct one
    # Might need to constrain E to have 2 singular values equal to one and the third to be zero unless cv2 does this
    U, S, Vt = np.linalg.svd(essential_mtx)
    W = np.array([[0, -1, 0], [1, 0, 0], [0, 0, 1]])
    u3 = U[:, 2].reshape(-1,1)
    get_R_matrix = lambda U, W, Vt: (U @ W @ Vt) / np.linalg.det(U @ W @ Vt) 
    # Determinant of R needs to be 1
    # note that sign(det(UWVt)) = det(UWVt)
    P_candidates = []
    R1 = get_R_matrix(U,W,Vt)
    R2 = get_R_matrix(U,W.T,Vt)
    P_candidates.append(np.block([[R1, u3]]))
    P_candidates.append(np.block([[R1, -u3]]))
    P_candidates.append(np.block([[R2, u3]]))
    P_candidates.append(np.block([[R2, -u3]]))
    skew = lambda x: np.array([[0, -x[2], x[1]], [x[2], 0, -x[0]], [-x[1], x[0], 0]])
    # Transform points to homogeneous coordinates and get skew symmetric matrices
    # Normalize points?
    # Normalize coordinates

    x = np.linalg.inv(intrinsics) @ np.append(points2d_1[0],1).reshape(-1,1)
    x_prime = np.linalg.inv(intrinsics) @ np.append(points2d_2[0],1).reshape(-1,1)
    x_skew = skew(x.reshape(-1))
    x_prime_skew = skew(x_prime.reshape(-1))
    # Form A matrix. Disregard the last row as it is a lin. combination of the first two
    A = np.zeros((4,4))
    A[0:2,:] = (x_skew @ P)[0:2,:]
    for P_prime in P_candidates:
        A[2:4,:] = (x_prime_skew @ P_prime)[0:2,:]
        X = np.linalg.svd(A)[2][-1]
        X = X / X[-1]
        q = X[-2] # This is the last value of PX. Determinant of M is 1
        q_prime = P_prime[2,0:4] @ X # This is the last value of P_prime X. Determinant of M_prime is 1
        # There should be ONLY one point in front of both cameras. This should be verified
        if q > 0 and q_prime > 0:
            extrinsics2 = P_prime
            break
        # Could compute the depth, but only need it to be positive. And the depth is given by
        # w sign(det(M))/(T*norm(m_3))
        # The sign of the derminant is thus unecessary also. Could just do the determinant
    extrinsics1 = P
    """ END YOUR CODE HERE """
...
def get_reprojection_residuals(points2d: np.ndarray, points3d: np.ndarray, intrinsics: np.ndarray,
    ...
    """ YOUR CODE HERE """
    P = intrinsics @ np.block([[rotation_mtx, tvec.reshape(-1, 1)]])
    points_2d_prime = P@(np.concatenate([points3d.T,np.ones((1,points3d.shape[0]))],axis=0))
    # Normalize points
    points_2d_prime = points_2d_prime / points_2d_prime[-1,:]
    residuals = np.linalg.norm(points2d - points_2d_prime[0:2,:].T, axis=1)
    """ END YOUR CODE HERE """
    return residuals
...
def solve_pnp(image_id: str, point2d_idxs: np.ndarray, all_points3d: np.ndarray, point3d_idxs: np.ndarray,
    ...
    for _ in range(num_ransac_iterations):
        ...
        """
        ret, rvec, tvec = cv2.solvePnP(selected_pts3d, selected_pts2d, intrinsics, None, flags=cv2.SOLVEPNP_ITERATIVE)
        if not ret:
            print("Failed to solve PnP")
            exit(0)
        rotation_mtx, _ = cv2.Rodrigues(rvec)
        residuals = get_reprojection_residuals(points2d=points2d, points3d=points3d, intrinsics=intrinsics,
                                               rotation_mtx=rotation_mtx, tvec=tvec)
        """ END YOUR CODE HERE """
...
def add_points3d(image_id1: str, image_id2: str, all_extrinsic: dict, intrinsics, points3d: np.ndarray,
    ...
    """
    new_points3d = triangulate(image_id1, image_id2, matches[:, 0], matches[:, 1], all_extrinsic[image_id1],
                               all_extrinsic[image_id2], intrinsics)
    """ END YOUR CODE HERE """
...
def get_next_pair(scene_graph: dict, registered_ids: list):
    ...
    """ YOUR CODE HERE """
    for im_id in registered_ids:
        for im_id_match in scene_graph[im_id]:
            if im_id_match in registered_ids:
                continue
            match_id = '_'.join(sorted([im_id, im_id_match]))
            match_file = os.path.join(RANSAC_MATCH_DIR, match_id + '.npy')
            matches = np.load(match_file)
            if len(matches) > max_num_inliers:
                max_num_inliers = len(matches)
                max_new_id = im_id_match
                max_registered_id = im_id
    """ END YOUR CODE HERE """
